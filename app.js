...
function renderCartSheet(){
  ...
  if(checkoutStep==='cart'){
    ...
    document.getElementById('goCheckout').onclick = ()=>{
      ...
      checkoutStep='form'; renderCartSheet();
    };
  } else {
...
function renderProfileForm(existing){
  const sheet = document.getElementById('menuSheet');
  sheet.innerHTML = `
    <div class="sheet-head"><h2>${existing?'تعديل بياناتي':'تسجيل الدخول / إنشاء حساب'}</h2><button class="close-x" id="closeMenuX">✕</button></div>
    <p style="font-size:12.5px;color:#6a5a4c;margin-bottom:10px;">سجّل دخولك بإيميلك أو رقم موبايلك مرة واحدة بس، وهتتعبى بياناتك تلقائياً في كل طلب جديد — من غير باسورد.</p>
    ...
    <button class="primary-btn" id="saveProfileBtn">${existing?'حفظ التعديلات':'تسجيل الدخول'}</button>
  `;
  document.getElementById('closeMenuX').onclick = closeMenu;
  document.getElementById('saveProfileBtn').onclick = ()=>{
    ...
    if(!email && !phone){ toast('اكتب إيميلك أو رقم موبايلك على الأقل'); return; }
