  ...
  grid.innerHTML = list.map(p=>{
    ...
    <div class="card" data-id="${p.id}">
      <div class="card-img" data-open="${p.id}">
        ...
        <button class="wish-btn ${wished?'active':''}" data-wish="${p.id}"
          aria-label="${wished?'إزالة من المفضلة':'إضافة للمفضلة'}"
          title="${wished?'في المفضلة ❤️':'أضف للمفضلة 🤍'}">${wished?'❤️':'🤍'}</button>
      </div>
      <div class="card-body">
...
function openProduct(id){
  ...
  document.getElementById('productSheet').innerHTML = `
    ...
    <div style="display:flex;gap:8px;">
      ...
      <button class="secondary-btn" id="detailWishBtn" style="flex:1;margin-top:8px;">${isWished(p.id)?'❤️ في المفضلة':'🤍 المفضلة'}</button>
  ...
  document.getElementById('detailWishBtn').onclick = ()=>{
    ...
    document.getElementById('detailWishBtn').textContent = nowWished ? '❤️ في المفضلة' : '🤍 المفضلة';
    toast(nowWished ? 'أُضيف للمفضلة ❤️' : 'أُزيل من المفضلة');
    renderProducts();
  };
...
function openCart(){
  if(!getProfile()){
    showLoginGate();
    toast('سجّل دخولك أولاً لإتمام الطلب');
    return;
  }
  checkoutStep='cart';
  renderCartSheet();
...
function openAccount(){
  ...
  if(profile){
    ...
    document.getElementById('logoutProfileBtn').onclick = ()=>{
      clearProfile();
      closeMenu();
      toast('تم تسجيل الخروج');
      setTimeout(showLoginGate, 150);
    };
...
function renderProfileForm(existing){
  ...
  // لو المستخدم مسجل بالفعل، يظل النموذج مخصصاً لتعديل البيانات.
  // لو مش مسجل، يتم الدخول بإيميل أو رقم موبايل — واحد منهما يكفي.
  sheet.innerHTML = `
    <div class="sheet-head">
      <h2>${existing?'تعديل بياناتي':'تسجيل الدخول / إنشاء حساب'}</h2>
      <button class="close-x" id="closeMenuX">✕</button>
    </div>
    <p style="font-size:12.5px;color:#6a5a4c;margin-bottom:10px;line-height:1.8;">
      ${existing
        ? 'عدّل بياناتك المحفوظة على الجهاز.'
        : 'ادخل باستخدام البريد الإلكتروني أو رقم الموبايل. مش لازم تكتب الاتنين.'}
    </p>
    <div class="field">
      <label>الاسم</label>
      <input type="text" id="prof_name" value="${existing?existing.name:''}" placeholder="اكتب اسمك">
    </div>
    <div class="field">
      <label>البريد الإلكتروني ${existing?'':'(اختياري إذا استخدمت الموبايل)'}</label>
      <input type="email" id="prof_email" value="${existing?existing.email||'':''}" placeholder="example@email.com">
    </div>
    <div class="field">
      <label>رقم الموبايل ${existing?'':'(اختياري إذا استخدمت الإيميل)'}</label>
      <input type="tel" id="prof_phone" value="${existing?existing.phone||'':''}" placeholder="01xxxxxxxxx">
    </div>
    <button class="primary-btn" id="saveProfileBtn">
      ${existing?'حفظ التعديلات':'دخول / إنشاء حساب'}
    </button>
  `;
  ...
  document.getElementById('saveProfileBtn').onclick = ()=>{
    ...
    if(!email && !phone){
      toast('اكتب الإيميل أو رقم الموبايل');
      return;
    }
    if(email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){
      toast('اكتب إيميل صحيح');
      return;
    }
    if(phone && !/^01[0-9]{9}$/.test(phone.replace(/\s/g,''))){
      toast('اكتب رقم موبايل مصري صحيح');
      return;
    }

    saveProfile({name, email, phone, loginMethod: email ? 'email' : 'phone', loggedInAt: new Date().toISOString()});
    toast(existing ? 'تم حفظ التعديلات' : 'تم تسجيل الدخول بنجاح');
    openAccount();
  };
}
...
function injectAuthStyles(){
  if(document.getElementById('sweethomeAuthStyles')) return;

  const style = document.createElement('style');
  style.id = 'sweethomeAuthStyles';
  style.textContent = `
    .sh-auth-overlay{
      position:fixed;inset:0;z-index:99999;
      background:rgba(59,36,23,.72);
      display:flex;align-items:center;justify-content:center;
      padding:18px;backdrop-filter:blur(5px);
    }
    .sh-auth-card{
      width:min(430px,100%);background:#fffaf2;
    ...
    .sh-auth-title{text-align:center;margin:0 0 5px;color:#3B2417;font-size:24px}
    .sh-auth-sub{text-align:center;color:#6a5a4c;font-size:13px;line-height:1.7;margin:0 0 18px}
    .sh-auth-methods{display:flex;gap:8px;margin-bottom:12px}
    .sh-auth-method{
      flex:1;border:1px solid rgba(179,18,46,.22);
      background:#fff;border-radius:12px;padding:10px;
      cursor:pointer;font-weight:800;color:#6a5a4c;
    }
    .sh-auth-method.active{background:var(--primary,#B3122E);color:#fff}
    .sh-auth-note{
      font-size:11.5px;color:#7a6a5c;line-height:1.7;
      background:rgba(200,155,60,.10);padding:9px 11px;border-radius:12px;margin-top:10px
    }
  `;
...
function showLoginGate(){
  ...
  let overlay = document.getElementById('sweethomeAuthOverlay');
  if(!overlay){
    overlay = document.createElement('div');
    overlay.id = 'sweethomeAuthOverlay';
    overlay.className = 'sh-auth-overlay';
  ...
  const render = ()=>{
    overlay.innerHTML = `
      <div class="sh-auth-card">
        <img class="sh-auth-logo" src="${s.logo||'logo.png'}" alt="Sweet Home">
        <h2 class="sh-auth-title">أهلاً بيك في ${s.storeName||'Sweet Home'} 🍰</h2>
        <p class="sh-auth-sub">سجّل دخولك علشان تقدر تستخدم التطبيق والمفضلة والطلبات.</p>

        <div class="sh-auth-methods">
          <button type="button" class="sh-auth-method ${method==='phone'?'active':''}" id="authPhoneTab">📱 رقم الهاتف</button>
          <button type="button" class="sh-auth-method ${method==='email'?'active':''}" id="authEmailTab">📧 الإيميل</button>
        </div>

        <div class="field">
          <label>الاسم</label>
          <input type="text" id="authName" placeholder="اكتب اسمك">
        </div>

        ${method==='phone' ? `
          <div class="field">
            <label>رقم الموبايل</label>
            <input type="tel" id="authPhone" placeholder="01xxxxxxxxx" inputmode="tel">
          </div>
        ` : `
          <div class="field">
            <label>البريد الإلكتروني</label>
            <input type="email" id="authEmail" placeholder="example@email.com" inputmode="email">
          </div>
        `}

        <button class="primary-btn" id="authSubmitBtn">دخول / إنشاء حساب</button>
        <div class="sh-auth-note">
          سيتم حفظ بيانات الحساب على هذا الجهاز. هذا الإصدار من التطبيق يستخدم تسجيل دخول محلي؛
          التحقق الحقيقي بكود SMS أو كود على الإيميل يحتاج ربط Backend/Firebase/Supabase.
        </div>
      </div>
    `;

    document.getElementById('authPhoneTab').onclick = ()=>{ method='phone'; render(); };
    document.getElementById('authEmailTab').onclick = ()=>{ method='email'; render(); };
    ...
    document.getElementById('authSubmitBtn').onclick = ()=>{
      ...
      if(method==='phone'){
        const phone = document.getElementById('authPhone').value.trim().replace(/\s/g,'');
...
function closeLoginGate(){
  const overlay = document.getElementById('sweethomeAuthOverlay');
...
async function loadSharedDataThenRender(){
  ...
  // أول دخول: لا يمكن استخدام التطبيق قبل تسجيل الحساب بالإيميل أو رقم الهاتف.
